...
public:
    ...
    friend vector& maximul_vectorilor(vector& data1, vector& data2);
};
...
istream& operator>>(istream& in, vector& data)
{
    data.citire(in);
    return in;
}
...
vector& maximul_vectorilor(vector &data1, vector &data2)
{
    if (data1.n != data2.n)
    {
...
void menu()
{
    ...
    do
    {
        ...
        } else if (option == 8)
        {
            ...
            for (i = 1; i < nr; ++i)
            {
                vector_maxim = maximul_vectorilor(vector_maxim, data[i]);
